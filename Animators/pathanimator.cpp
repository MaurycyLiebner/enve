...
void PathAnimator::updatePathPointIds()
{
    int pointId = 0;
...
PathPoint *PathAnimator::addPointRelPos(const QPointF &relPos,
...
bool PathAnimator::getTAndPointsForMouseEdgeInteraction(const QPointF &absPos,
                                                      qreal *pressedT,
                                                      PathPoint **prevPoint,
                                                      PathPoint **nextPoint) {
    const QMatrix &combinedTransform = mParentBox->getCombinedTransform();
    ...
    QPointF relPos = mParentBox->mapAbsPosToRel(absPos);
...
PathPoint *PathAnimator::createNewPointOnLineNear(QPointF absPos,
                                                bool adjust) {
    qreal pressedT;
    PathPoint *prevPoint = NULL;
    PathPoint *nextPoint = NULL;
...
void PathAnimator::updateAfterFrameChanged(int currentFrame) {
    foreach(PathPoint *point, mPoints) {
...
qreal PathAnimator::findPercentForPoint(QPointF point,
                                      PathPoint **prevPoint,
                                      qreal *error) {
...
void PathAnimator::applyTransformToPoints(QMatrix transform) {
    foreach(PathPoint *point, mPoints) {
        point->applyTransform(transform);
    }
}

void PathAnimator::disconnectPoints(PathPoint *point1,
                                  PathPoint *point2) {
...
void PathAnimator::connectPoints(PathPoint *point1,
                                 PathPoint *point2) {
...
void PathAnimator::deletePointAndApproximate(PathPoint *pointToRemove) {
...
void PathAnimator::addPointToSeparatePaths(PathPoint *pointToAdd,
                                           const bool &saveUndoRedo) {
    mSeparatePaths.append(pointToAdd);
    pointToAdd->setSeparatePathPoint(true);

    if(saveUndoRedo) {
        AddPointToSeparatePathsUndoRedo *undoRedo =
                new AddPointToSeparatePathsUndoRedo(this, pointToAdd);
...
void PathAnimator::removePointFromSeparatePaths(PathPoint *pointToRemove,
                                              bool saveUndoRedo) {
    pointToRemove->setSeparatePathPoint(false);
    mSeparatePaths.removeOne(pointToRemove);

    if(saveUndoRedo) {
        RemovePointFromSeparatePathsUndoRedo *undoRedo =
                new RemovePointFromSeparatePathsUndoRedo(this, pointToRemove);
        addUndoRedo(undoRedo);
    }
    //schedulePathUpdate();
    callUpdater();
...
PathPoint *PathAnimator::addPoint(PathPoint *pointToAdd, PathPoint *toPoint)
{
    if(toPoint == NULL) {
...
PathPoint* PathAnimator::addPointAbsPos(const QPointF &absPtPos,
                                        PathPoint *toPoint)
{
    PathPoint *newPoint = new PathPoint(this);
...
PathPoint *PathAnimator::addPointRelPos(const QPointF &relPtPos,
                                        PathPoint *toPoint)
{
    PathPoint *newPoint = new PathPoint(this);
...
void PathAnimator::appendToPointsList(PathPoint *point,
                                      const bool &saveUndoRedo) {
...
void PathAnimator::removeFromPointsList(PathPoint *point,
                                        const bool &saveUndoRedo) {
...
void PathAnimator::removePoint(PathPoint *point) {
...
void PathAnimator::replaceSeparatePathPoint(PathPoint *pointBeingReplaced,
                                            PathPoint *newPoint) {
    removePointFromSeparatePaths(pointBeingReplaced);
    addPointToSeparatePaths(newPoint);
}

void PathAnimator::startAllPointsTransform()
{
    foreach(PathPoint *point, mPoints) {
...
void PathAnimator::finishAllPointsTransform() {
    foreach(PathPoint *point, mPoints) {
        point->finishTransform();
    }
}

void PathAnimator::duplicatePathPointsTo(
        PathAnimator *target) {
