...
void Canvas::saveSceneJSON(JsonExporter& exp) const {
    if(exp.fBackground) {
        auto bg = exp.createLayer(JsonLayerType::shape);
        bg.insert("bm", 0);
        bg.insert("ip", 0);
        bg.insert("op", exp.fAbsRange.span() - 1);

        QJsonObject bgTransform;

        QJsonObject bgAnchor;
        bgAnchor.insert("a", 0);
        bgAnchor.insert("k", QJsonArray() << 0 << 0 << 0);
        bgTransform.insert("a", bgAnchor);

        QJsonObject bgOpacity;
        bgOpacity.insert("a", 0);
        bgOpacity.insert("k", 100);
        bgTransform.insert("o", bgOpacity);

        QJsonObject bgPosition;
        bgPosition.insert("a", 0);
        bgPosition.insert("k", QJsonArray() << 0 << 0 << 0);
        bgTransform.insert("p", bgPosition);

        QJsonObject bgRotation;
        bgRotation.insert("a", 0);
        bgRotation.insert("k", 0);
        bgTransform.insert("r", bgRotation);

        QJsonObject bgScale;
        bgScale.insert("a", 0);
        bgScale.insert("k", QJsonArray() << 200 << 200 << 200);
        bgTransform.insert("s", bgScale);

        bg.insert("ks", bgTransform);

        QJsonObject bgRect;
        bgRect.insert("ty", "gr");

        QJsonObject bgShape;
        bgShape.insert("ty", "rc");
        bgShape.insert("d", 1);
        QJsonObject bgP;
        bgP.insert("a", 0);
        bgP.insert("k", QJsonArray() << 0 << 0);
        bgShape.insert("p", bgP);
        QJsonObject bgS;
        bgS.insert("a", 0);
        bgS.insert("k", QJsonArray() << mWidth << mHeight);
        bgShape.insert("s", bgS);
        ...
        bgO.insert("k", 100);
        bgFill.insert("o", bgO);
        QJsonObject bgC = mBackgroundColor->saveColorJSON(exp, exp.fAbsRange);
        bgFill.insert("c", bgC);

        bgRect.insert("it", QJsonArray() << bgShape << bgFill);


        bg.insert("shapes", QJsonArray() << bgRect);

        exp.addLayer(std::move(bg));
    }

    const auto task = enve::make_shared<JsonEleTask>(exp, exp.fAbsRange);
    exp.addNextTask(task);
    saveBoxesJSON(exp, task.get(), task->element());
    task->queTask();
}
