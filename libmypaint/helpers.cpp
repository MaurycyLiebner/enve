...
static const double spectral_r_small[10] = {0.009281362787953,0.009732627042016,0.011254252737167,0.015105578649573
,0.024797924177217,0.083622585502406,0.977865045723212,1.000000000000000
,0.999961046144372,0.999999992756822};

static const double spectral_g_small[10] = {0.002854127435775,0.003917589679914,0.012132151699187,0.748259205918013
,1.000000000000000,0.865695937531795,0.037477469241101,0.022816789725717
,0.021747419446456,0.021384940572308};

static const double spectral_b_small[10] = {0.537052150373386,0.546646402401469,0.575501819073983,0.258778829633924
...
// C fmod function is not "arithmetic modulo"; it doesn't handle negative dividends as you might expect
...
rgb_to_hcy_double (double *r_, double *g_, double *b_) {
    ...
    double _HCY_RED_LUMA = 0.2162;
    double _HCY_GREEN_LUMA = 0.7152;
    double _HCY_BLUE_LUMA = 0.0722;
    double h, c, y;
    double r, g, b;
    double p, n, d;
...
spectral_to_rgb (double *spectral, double *rgb_) {
  ...
  for (int i=0; i<3; i++) {
    rgb_[i] = CLAMP((rgb_[i] - WGM_EPSILON) / offset, 0., 1.);
  }
}
...
{
  static double result[4] = {0};
  
  double opa_a = fac;
  double opa_b = 1.0-opa_a;
  result[3] = CLAMP(opa_a * a[3] + opa_b * b[3], 0., 1.);
  double sfac_a = opa_a * a[3] / (a[3] + b[3] * opa_b);
  double sfac_b = 1 - sfac_a;
  ...
  if (paint_mode > 0.0) { 
    ...
    double spectralmix[10] = {0};
    for (int i=0; i < 10; i++) {
      spectralmix[i] = pow(spec_a[i], sfac_a) * pow(spec_b[i], sfac_b);
    }
    
    //convert to RGB
    double rgb_result[3] = {0};
