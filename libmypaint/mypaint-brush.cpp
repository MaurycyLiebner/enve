...
double smudge_buckets[256][9] = {{0.0}};
...
struct MyPaintBrush {
    ...
    double states[MYPAINT_BRUSH_STATES_COUNT];
    double random_input;
    double skip;
    double skip_last_x;
    double skip_last_y;
    double skipped_dtime;
    RngDouble * rng;
    ...
    double settings_value[MYPAINT_BRUSH_SETTINGS_COUNT];

    // see also brushsettings.py

    // cached calculation results
    double speed_mapping_gamma[2];
    double speed_mapping_m[2];
    double speed_mapping_q[2];
...
mypaint_brush_set_state(MyPaintBrush *self, MyPaintBrushState i, double value)
{
  ...
  double exp_decay (double T_const, double t)
  {
  ...
  void settings_base_values_have_changed (MyPaintBrush *self)
  {
    ...
    for (i=0; i<2; i++) {
      ...
      double fix1_x, fix1_y, fix2_x, fix2_dy;
      fix1_x = 45.0;
      fix1_y = 0.5;
      fix2_x = 45.0;
      fix2_dy = 0.015;
  ...
  void update_states_and_setting_values (MyPaintBrush *self, double step_ddab, double step_dx, double step_dy, double step_dpressure, double step_declination, double step_ascension, double step_dtime, double step_viewzoom, double step_viewrotation, double step_declinationx, double step_declinationy, double step_barrel_rotation)
  {
    double pressure;
    double inputs[MYPAINT_BRUSH_INPUTS_COUNT];
    double viewzoom;
    double viewrotation;
    double gridmap_scale;
    double gridmap_scale_x;
    double gridmap_scale_y;
    ...
    double base_radius = exp(mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_RADIUS_LOGARITHMIC]));
    ...
    double norm_dx, norm_dy, norm_dist, norm_speed;
    ...
    norm_speed = hypot(norm_dx, norm_dy);
    //norm_dist should relate to brush size, whereas norm_speed should not
    norm_dist = hypot(step_dx / step_dtime / base_radius, step_dy / step_dtime / base_radius) * step_dtime;

    inputs[MYPAINT_BRUSH_INPUT_PRESSURE] = pressure * exp(mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_PRESSURE_GAIN_LOG]));
    ...
    inputs[MYPAINT_BRUSH_INPUT_DIRECTION] = mod_arith(atan2 (self->states[MYPAINT_BRUSH_STATE_DIRECTION_DY], self->states[MYPAINT_BRUSH_STATE_DIRECTION_DX])/(2*M_PI)*360 + self->states[MYPAINT_BRUSH_STATE_VIEWROTATION] + 180.0, 180.0);
    inputs[MYPAINT_BRUSH_INPUT_DIRECTION_ANGLE] = fmod(atan2(self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DY], self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DX]) / (2 * M_PI) * 360 + self->states[MYPAINT_BRUSH_STATE_VIEWROTATION] + 360.0, 360.0) ;
    ...
    inputs[MYPAINT_BRUSH_INPUT_ATTACK_ANGLE] = smallest_angular_difference(self->states[MYPAINT_BRUSH_STATE_ASCENSION], mod_arith(atan2(self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DY], self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DX]) / (2 * M_PI) * 360 + 90, 360));
    ...
    inputs[MYPAINT_BRUSH_INPUT_ATTACK_ANGLE] = smallest_angular_difference(self->states[MYPAINT_BRUSH_STATE_ASCENSION], mod_arith(atan2(self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DY], self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DX]) / (2 * M_PI) * 360 + 90 + self->states[MYPAINT_BRUSH_STATE_VIEWROTATION], 360));
    ...
    {
      double fac = 1.0 - exp_decay (self->settings_value[MYPAINT_BRUSH_SETTING_SLOW_TRACKING_PER_DAB], step_ddab);
    ...
    { // slow speed, but as vector this time
      ...
      double time_constant = exp(self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_BY_SPEED_SLOWNESS]*0.01)-1.0;
      ...
      double fac = 1.0 - exp_decay (time_constant, step_dtime);
    ...
    { // orientation (similar lowpass filter as above, but use dabtime instead of wallclock time)
      //adjust speed with viewzoom
      double dx = step_dx *self->states[MYPAINT_BRUSH_STATE_VIEWZOOM];
      double dy = step_dy *self->states[MYPAINT_BRUSH_STATE_VIEWZOOM];



      double step_in_dabtime = hypot(dx, dy); // FIXME: are we recalculating something here that we already have?
      double fac = 1.0 - exp_decay (exp(self->settings_value[MYPAINT_BRUSH_SETTING_DIRECTION_FILTER]*0.5)-1.0, step_in_dabtime);

      double dx_old = self->states[MYPAINT_BRUSH_STATE_DIRECTION_DX];
      double dy_old = self->states[MYPAINT_BRUSH_STATE_DIRECTION_DY];
    ...
    { // stroke length
      double frequency;
      double wrap;
      frequency = exp(-self->settings_value[MYPAINT_BRUSH_SETTING_STROKE_DURATION_LOGARITHMIC]);
      ...
      if (self->states[MYPAINT_BRUSH_STATE_STROKE] > wrap) {
        if (wrap > 9.9 + 1.0) {
          ...
          self->states[MYPAINT_BRUSH_STATE_STROKE] = fmod(self->states[MYPAINT_BRUSH_STATE_STROKE], wrap);
          // just in case
          if (self->states[MYPAINT_BRUSH_STATE_STROKE] < 0) self->states[MYPAINT_BRUSH_STATE_STROKE] = 0;
        }
      }
    ...
    self->states[MYPAINT_BRUSH_STATE_ACTUAL_RADIUS] = exp(radius_log);
  ...
  gboolean prepare_and_draw_dab (MyPaintBrush *self, MyPaintSurface * surface)
  {
    ...
    if (self->settings_value[MYPAINT_BRUSH_SETTING_OPAQUE_LINEARIZE]) {
      ...
      double alpha, beta, alpha_dab, beta_dab;
      double dabs_per_pixel;
    ...
    double base_radius = exp(mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_RADIUS_LOGARITHMIC]));
    double offset_mult = exp(self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_MULTIPLIER]);
    ...
    if (self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_ANGLE]) {
      x += cos((fmod ((atan2(self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DY], self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DX]) ) / (2 * M_PI) * 360 - 90, 360.0) + self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_ANGLE_ADJ])* M_PI / 180) * base_radius * self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_ANGLE] * offset_mult;
      y += sin((fmod ((atan2(self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DY], self->states[MYPAINT_BRUSH_STATE_DIRECTION_ANGLE_DX]) ) / (2 * M_PI) * 360 - 90, 360.0) + self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_ANGLE_ADJ])* M_PI / 180) * base_radius * self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_ANGLE] * offset_mult;
    }
    ...
    if (self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_BY_RANDOM]) {
      double amp = self->settings_value[MYPAINT_BRUSH_SETTING_OFFSET_BY_RANDOM];
      if (amp < 0.0) amp = 0.0;
    ...
    if (self->settings_value[MYPAINT_BRUSH_SETTING_RADIUS_BY_RANDOM]) {
      double radius_log, alpha_correction;
    ...
    // color part
    double color_h = mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_COLOR_H]);
    double color_s = mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_COLOR_S]);
    double color_v = mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_COLOR_V]);
    ...
    if (self->settings_value[MYPAINT_BRUSH_SETTING_SMUDGE_LENGTH] < 1.0 &&
      ...
      double fac = self->settings_value[MYPAINT_BRUSH_SETTING_SMUDGE_LENGTH];
      ...
      if (smudge_buckets[bucket][8] < (pow(0.5*fac, self->settings_value[MYPAINT_BRUSH_SETTING_SMUDGE_LENGTH_LOG])) + 0.0000000000000001) {
        ...
        double smudge_radius = radius * exp(self->settings_value[MYPAINT_BRUSH_SETTING_SMUDGE_RADIUS_LOG]);
      ...
      double smudge_state[4] = {smudge_buckets[bucket][0], smudge_buckets[bucket][1], smudge_buckets[bucket][2], smudge_buckets[bucket][3]};
      
      double smudge_get[4] = {r, g, b, a};
    ...
    double eraser_target_alpha = 1.0;

    if (self->settings_value[MYPAINT_BRUSH_SETTING_SMUDGE] > 0.0) {
      double fac = self->settings_value[MYPAINT_BRUSH_SETTING_SMUDGE];
      //hsv_to_rgb_double (&color_h, &color_s, &color_v);
    ...
    double hardness = CLAMP(self->settings_value[MYPAINT_BRUSH_SETTING_HARDNESS], 0.0, 1.0);

    // anti-aliasing attempt (works surprisingly well for ink brushes)
    double current_fadeout_in_pixels = radius * (1.0 - hardness);
    double min_fadeout_in_pixels = self->settings_value[MYPAINT_BRUSH_SETTING_ANTI_ALIASING];
    if (current_fadeout_in_pixels < min_fadeout_in_pixels) {
      ...
      double current_optical_radius = radius - (1.0-hardness)*radius/2.0;
      ...
      double hardness_new = ((current_optical_radius - (min_fadeout_in_pixels/2.0))/(current_optical_radius + (min_fadeout_in_pixels/2.0)));
      // Using Equation 1:
      double radius_new = (min_fadeout_in_pixels/(1.0 - hardness_new));
    ...
    // snap to pixel
    double snapToPixel = self->settings_value[MYPAINT_BRUSH_SETTING_SNAP_TO_PIXEL];
    if (snapToPixel > 0.0)
    {
      // linear interpolation between non-snapped and snapped
      double snapped_x = floor(x) + 0.5;
      double snapped_y = floor(y) + 0.5;
  ...
  double count_dabs_to (MyPaintBrush *self, double x, double y, double pressure, double dt)
  {
    double xx, yy;
    double res1, res2, res3;
    double dist;

    if (self->states[MYPAINT_BRUSH_STATE_ACTUAL_RADIUS] == 0.0) self->states[MYPAINT_BRUSH_STATE_ACTUAL_RADIUS] = exp(mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_RADIUS_LOGARITHMIC]));
    if (self->states[MYPAINT_BRUSH_STATE_ACTUAL_RADIUS] < ACTUAL_RADIUS_MIN) self->states[MYPAINT_BRUSH_STATE_ACTUAL_RADIUS] = ACTUAL_RADIUS_MIN;
    if (self->states[MYPAINT_BRUSH_STATE_ACTUAL_RADIUS] > ACTUAL_RADIUS_MAX) self->states[MYPAINT_BRUSH_STATE_ACTUAL_RADIUS] = ACTUAL_RADIUS_MAX;
    ...
    if (self->states[MYPAINT_BRUSH_STATE_ACTUAL_ELLIPTICAL_DAB_RATIO] > 1.0) {
      ...
      double angle_rad=self->states[MYPAINT_BRUSH_STATE_ACTUAL_ELLIPTICAL_DAB_ANGLE]/360*2*M_PI;
      double cs=cos(angle_rad);
      double sn=sin(angle_rad);
      double yyr=(yy*cs-xx*sn)*self->states[MYPAINT_BRUSH_STATE_ACTUAL_ELLIPTICAL_DAB_RATIO];
      double xxr=yy*sn+xx*cs;
  ...
  int mypaint_brush_stroke_to (MyPaintBrush *self, MyPaintSurface *surface,
                                ...
                                double xtilt, double ytilt, double dtime, double viewzoom, double viewrotation, double barrel_rotation)
  {
    const double max_dtime = 5;

    //printf("%f %f %f %f\n", (double)dtime, (double)x, (double)y, (double)pressure);

    double tilt_ascension = 0.0;
    double tilt_declination = 90.0;
    double tilt_declinationx = 90.0;
    double tilt_declinationy = 90.0;
    if (xtilt != 0 || ytilt != 0) {
    ...
    */
    ...
    if (self->skip > 0.001) {
      double dist = hypot(self->skip_last_x-x, self->skip_last_y-y);
      self->skip_last_x = x;
      self->skip_last_y = y;
    ...
    { // calculate the actual "virtual" cursor position
      ...
      if (mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_TRACKING_NOISE])) {
        // OPTIMIZE: fastexp() called too often
        const double base_radius = exp(mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_RADIUS_LOGARITHMIC]));
        const double noise = base_radius * mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_TRACKING_NOISE]);
      ...
      const double fac = 1.0 - exp_decay (mypaint_mapping_get_base_value(self->settings[MYPAINT_BRUSH_SETTING_SLOW_TRACKING]), 100.0*dtime);
    ...
    double dabs_moved = self->states[MYPAINT_BRUSH_STATE_PARTIAL_DABS];
    double dabs_todo = count_dabs_to (self, x, y, pressure, dtime);
    ...
    double step_ddab, step_dx, step_dy, step_dpressure, step_dtime;
    double step_declination, step_ascension, step_declinationx, step_declinationy, step_viewzoom, step_viewrotation, step_barrel_rotation;

    while (dabs_moved + dabs_todo >= 1.0) { // there are dabs pending
      { // linear interpolation (nonlinear variant was too slow, see SVN log)
        double frac; // fraction of the remaining distance to move
        if (dabs_moved > 0) {
                                      ...
                                      json_object *setting_obj)
{
    ...
    json_object_object_foreach(inputs, input_name, input_obj) {
        ...
        for (int i=0; i<number_of_mapping_points; i++) {
            ...
            const double x = json_object_get_double(x_obj);
            json_object *y_obj = json_object_array_get_idx(mapping_point, 1);
            const double y = json_object_get_double(y_obj);
